"""Sweep eval across ALL checkpoints for v3 (32 attempts).
...
Usage:
  modal run eval_sweep_7b_v3.py
  modal run eval_sweep_7b_v3.py --num-eval-problems 50
"""
import modal

app = modal.App("cs224n-7b-eval-sweep-v3")
...
def eval_one_checkpoint(checkpoint_step: int, num_eval_problems: int = 50):
    ...
    logger = logging.getLogger(f"sweep_v3_step{checkpoint_step}")
    volume.reload()
    ...
    with open(f"{VOLUME_PATH}/sweep_v3_step{checkpoint_step}.json", "w") as f:
...
def eval_baseline(num_eval_problems: int = 50):
    ...
    logger = logging.getLogger("sweep_v3_baseline")
    ...
    with open(f"{VOLUME_PATH}/sweep_v3_baseline.json", "w") as f:
        json.dump(result, f, indent=2)
    volume.commit()
    return result
...
def main(num_eval_problems: int = 50, min_step: int = 0):
    """Sweep eval across v3 checkpoints.

    Usage:
      modal run eval_sweep_7b_v3.py
      modal run eval_sweep_7b_v3.py --num-eval-problems 30
      modal run eval_sweep_7b_v3.py --min-step 200   # only checkpoints after 200 (250, 300, ...)
    """
    print("=" * 100)
    print("  v3 CHECKPOINT SWEEP EVAL (32 attempts)")
    print("=" * 100)
    ...
    print("\n" + "=" * 100)
    print(f"  v3 CHECKPOINT PROGRESSION (baseline acc={base_acc:.4f}  solve={base_solve:.4f}  [{base_correct}/{base_n}])")
    print("=" * 100)
