"""7B SDPO v2 batch update — 16 sequential attempts, full attempt logging.

Key differences from sdpo_update_7b.py:
- Returns detailed rollout_log with every attempt's score/feedback/output snippet
...
# ------------------------------------------------------------------
# Sequential rollout with cumulative feedback (16 attempts)
# ------------------------------------------------------------------
...
def _sequential_rollout(agent_model, prompt, system_prompt, tests_json,
                        verify_fn, max_attempts=16,
                        temperature=0.7, max_new_tokens=1536,
                        rollout_idx=0, feedback_truncate=300,
                        feedback_window=6):
    """Generate one rollout with up to 16 sequential attempts.

    Uses a sliding window of the last `feedback_window` feedbacks
    to keep prompt length bounded.
    Returns best attempt + full attempt log.
    """
    best = None
    feedback_history = []
    attempt_log = []
    ...
    for attempt in range(max_attempts):
        ...
        if feedback_history:
            recent = feedback_history[-feedback_window:]
            start_idx = len(feedback_history) - len(recent)
            history_str = "\n".join(
        ...
        # Logging
        if attempt % 4 == 0 or is_perfect or attempt == max_attempts - 1:
            # Log every 4th attempt, perfect ones, and the last one
            ...
            for line in decoded[:1000].split('\n'):
                logger.info(f"      ║  {line}")
            if len(decoded) > 1000:
            ...
            if feedback:
                for line in feedback[:300].split('\n'):
                    logger.info(f"      ║    {line}")
            else:
                logger.info(f"      ║    All tests passed!")
            logger.info(f"      ╚══════════════════════════════════════════════════════════╝")
        else:
            # Brief log for intermediate attempts
        ...
        # Truncate feedback more aggressively for 16-attempt prompts
...
# ------------------------------------------------------------------
# Main 7B v2 SDPO batch step
# ------------------------------------------------------------------

def sdpo_batch_step_7b_v2(agent_model, batch_items, verify_fn,
                           config, sdpo_config=None):
    """SDPO gradient step for 7B v2 — 16 attempts, full logging."""
    if sdpo_config is None:
        sdpo_config = SelfDistillationConfig(reference_kl_beta=0.0)

    G = config.num_rollouts
    max_attempts = getattr(config, "max_sequential_attempts", 16)
    topk = sdpo_config.distillation_topk or 50
    feedback_truncate = getattr(config, "feedback_truncate", 300)
    feedback_window = getattr(config, "feedback_window", 6)

    logger.info(f"  ╔══════════════════════════════════════════════════════════════════╗")
    logger.info(f"  ║  7B v2 SDPO BATCH: {len(batch_items)} problems, {G} rollouts x "
    ...
    all_problem_data = []
    rollout_log = []  # Detailed log for all rollouts across all problems
    ...
    for item_idx, item in enumerate(batch_items):
        ...
        logger.info(f"\n  ┌{'─'*70}┐")
        logger.info(f"  │  7B v2 PROBLEM {item_idx+1}/{len(batch_items)} "
    ...
    logger.info(f"\n  ╔══════════════════════════════════════════════════════════════════╗")
    logger.info(f"  ║  7B v2 SDPO STEP COMPLETE                                      ║")
